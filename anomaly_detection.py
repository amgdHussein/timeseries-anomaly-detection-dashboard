...
class ADetector(object):
    ...
    def future_dataframe(self, period):
        self.period = period
        data = self.model.make_future_dataframe(periods=self.period, freq='D', include_history=True)
    ...
    def classify_anomaly(self, stds=[2, 4, 8]):
        # populate errors
        self.dataframe_predict['residuals'] = self.dataframe_predict['y'] - self.dataframe_predict['yhat']
    ...
    def stream_anomaly_plot(self, series_name=''):
        ...
        confidence_area = go.Scatter(
            x=pd.concat([self.dataframe_forecast.ds, self.dataframe_forecast.ds[::-1]]),
            y=pd.concat([self.dataframe_forecast.yhat_upper, self.dataframe_forecast.yhat_lower[::-1]]),
            mode='none',
        ...
        for i in range(len(colors)):
            anomalies = self.dataframe_predict[self.dataframe_predict.impact==i+1]
    ...
    error = lambda self, metric: metric(self.dataframe_predict.y, self.dataframe_predict.yhat)
    def metric_plot(self):
        ...
        fig.update_layout(
            title=dict(text='Error Metric Results', font=dict(size=20)),
            template='plotly_dark', 
            margin=dict(r=10, l=10, b=10),
        )
        return fig
